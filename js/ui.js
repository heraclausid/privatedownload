/* --- js/ui.js (FIXED: ANIMATION SYNC & SLIDER LIMITS) --- */
...
function renderStyleTab(el) {
    ...
    if (currentEditState === 'normal') {
        ...
        // [FIXED] Max 100 (10s) agar leluasa
    ...
    // --- TIPOGRAFI ---
    if (['heading', 'paragraph', 'button', 'theme-toggle', 'icon'].includes(el.type)) {
...
document.getElementById('saveEditBtn').onclick = () => { 
    ...
    contentInputs.forEach(inp => { if (inp.id.startsWith('picker-') || inp.id === 'input-grid-count' || inp.id === 'input-internal-selector') return; const parts = inp.id.split('-'); if (parts.length < 3) return; const cat = parts[1]; const key = parts[2]; let val = inp.value; if(['gap', 'thickness', 'fontSize'].includes(key)) val = parseFloat(val); if (val === '') val = undefined; if (cat === 'content') el.content[key] = val; else if(cat === 'layout') el.layout[key] = val; else if (cat === 'styles') el.styles[key] = val; }); 
    if (currentEditState === 'normal') {
        ...
        if (animType && animDur && animDel) { 
            ...
            // [FIXED] Hapus pembagian /10 disini. Input angka adalah nilai detik sebenarnya (misal ketik 6 = 6s)
...
function createSlider(k, l, v, min = 0, max = 100, unit = '') { 
    // Jika animasi, v yg masuk = detik * 10 (misal 60). Diinput kita bagi 10 jadi 6.0
    const displayVal = k.includes('animation') ? (v/10).toFixed(1) : v;
    return `<div class="prop-row">
        <div class="prop-label">${l}</div>
        <div class="prop-control" style="flex:2">
            <div class="slider-wrapper">
                <input type="range" min="${min}" max="${max}" value="${v||0}" class="slider-range" oninput="document.getElementById('input-${k}').value = this.id.includes('animation') ? (this.value/10).toFixed(1) : this.value">
                <div class="input-wrapper" style="width:75px">
                    <input type="number" id="input-${k}" value="${displayVal}" class="input-field" oninput="let val = this.value * (this.id.includes('animation')?10:1); this.closest('.slider-wrapper').querySelector('input[type=range]').value = val">
