...
function applyGlobalConfig() {
    ...
    const currentBg = globalConfig.pageBg || '#f1f5f9';
    const bgPicker = document.getElementById('global-bg');
    const bgText = document.getElementById('global-bg-text');

    if (bgPicker) bgPicker.value = currentBg;
    if (bgText) bgText.value = currentBg.toUpperCase();
    
    if (typeof renderGlobalPalette === 'function') renderGlobalPalette();
